...
def conv2d_bn(x,
    ...
    if not use_bias:
        ...
        x = tf.keras.layers.BatchNormalization(axis=bn_axis, momentum=0.995, epsilon=0.001,
...
def _inception_resnet_block(x, scale, block_type, block_idx, activation='relu'):
    ...
    up = tf.keras.layers.Lambda(scaling,
                                output_shape=tf.keras.backend.int_shape(up)[1:],
...
def InceptionResNetV1(input_shape=(160, 160, 3),
    ...
    branch_pool = tf.keras.layers.MaxPooling2D(3,
                                               strides=2,
                                               padding='valid',
    ...
    # Bottleneck
    x = tf.keras.layers.Dense(classes, use_bias=False, name='Bottleneck')(x)
    bn_name = _generate_layer_name('BatchNorm', prefix='Bottleneck')
    x = tf.keras.layers.BatchNormalization(momentum=0.995, epsilon=0.001, scale=False,
                                           name=bn_name)(x)
